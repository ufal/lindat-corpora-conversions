...
def create_sox_command(input_filename, output_filename, start_time, end_time,speech_input_dir,speech_output_dir):
    length = float(end_time)-float(start_time)
    return 'sox %s/%s.wav %s/%s trim %f %f' % (speech_input_dir,input_filename,speech_output_dir,output_filename, float(start_time), length)
...
def process_segment(filename,segment,speech_input_dir,speech_output_dir):
    (start_time, end_time, text) = segment
    tokens = tokenize_text(text)
    speech_filename = create_speech_filename(filename, start_time, end_time)
    sox = create_sox_command(filename, speech_filename, start_time, end_time,speech_input_dir,speech_output_dir)
    buf = ''
...
def proces_turn(filename,turn,speaker_map,speech_input_dir,speech_output_dir):
    ...
    for segment in segments:
        [mybuf,mysox] = process_segment(filename, segment,speech_input_dir,speech_output_dir)
        buf += mybuf
...
def process_tree(filename,tree,corpus_filename,sox_commands_filename,speech_input_dir,speech_output_dir):
    ...
    buf = ''
    for turn in turns:
        [mybuf, mycommands] = proces_turn(filename,turn,speaker_map,speech_input_dir,speech_output_dir)
        buf += mybuf
        commands += mycommands
...
def main():
    argv = sys.argv
    if len(argv) != 6:
        print "USAGE: %s <input trs file> <corpus file> <sox commands file> <speech input dir> <speech output dir>" % argv[0]
        return
    trs_filename = argv[1]
    corpus_filename = argv[2]
    sox_commands_filename = argv[3]
    speech_input_dir = argv[4]
    speech_output_dir = argv[5]
    basename = os.path.splitext(os.path.basename(trs_filename))[0]
    tree = ET.parse(trs_filename)
    process_tree(basename,tree,corpus_filename,sox_commands_filename,speech_input_dir,speech_output_dir)
